<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['community:likes:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['community:likes:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['community:likes:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['community:likes:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="likesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="点赞id" align="center" prop="likesId" />
...
<script>
import { listLikes, getLikes, delLikes, addLikes, updateLikes } from "@/api/community/likes";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.likesId)
    ...
    handleUpdate(row) {
      this.reset();
      const likesId = row.likesId || this.ids
      getLikes(likesId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const likesIds = row.likesId || this.ids;
      this.$modal.confirm('是否确认删除点赞管理编号为"' + likesIds + '"的数据项？').then(function() {
        return delLikes(likesIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('community/likes/export', {
        ...this.queryParams
